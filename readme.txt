...
🔥 **NEW IN PRO: Media Highlights Story Albums** — Create Facebook-style story albums that group your photos and videos into swipeable, autoplay stories with a stunning full-screen viewer!
...
- **Reusable Galleries (Shortcode):** Build a gallery once and place it anywhere—widgets, page-builder rows, or theme templates—using a simple shortcode `[image_gallery id="123"]`.
- **Ready-Made Sections:** Get started instantly with four beautiful block patterns: Photo album grid, Polaroid photo wall, Portfolio showcase, and Product lookbook.
...
- **Bulk Import from Media Library:** Select multiple photos at once and add them as gallery items or images in one step, instead of one at a time.
- **Albums & Filter Bar:** Group images into albums and let visitors filter the gallery with a clickable filter bar.
- **Image Watermarking:** Overlay a text or logo watermark on gallery and lightbox images, with position, opacity, and where it applies all configurable.
- **Load More Pagination:** Load a limited number of images up front and reveal the rest on demand, for faster-loading large galleries.
- **Lightbox Toolbar:** Fullscreen, download, and share/copy-link buttons plus an image counter in the lightbox popup.
- **Aspect Ratio Control:** Force gallery thumbnails to a square, 4:3, 16:9, 3:2, or 9:16 crop instead of a fixed height.
- **Right-Click Protection:** Optionally prevent visitors from right-clicking to save gallery images.
- **Keyboard & Screen Reader Accessible:** Gallery items and the lightbox are fully keyboard-operable, with proper ARIA roles and focus handling.
...
- **Media Highlights Story Albums:** Facebook-style story albums that group photos and videos, opening in a full-screen autoplay story viewer.
...
10. Media Highlights Story Albums
...
* **New: Bulk Add from Media Library.** Select several photos at once from the Media Library and add them all in one step — as new gallery items on the Default style, or new frames on the Polaroid wall — instead of adding, uploading, and browsing for each photo one at a time.
* **New: Albums & Filter Bar.** Group Default and Polaroid images into named albums and turn on a clickable filter bar so visitors can narrow the gallery down to one album at a time.
* **New: Image Watermarking.** Overlay a text or logo watermark on gallery thumbnails and/or the lightbox popup, with position, opacity, and color all configurable from the sidebar.
* **New: Load More Pagination.** Show a limited number of images up front with a "Load More" button to reveal the rest, so large galleries load faster.
* **New: Lightbox Toolbar.** The lightbox popup can now show a fullscreen toggle, a download button, a share/copy-link button, and an image counter (e.g. "3 / 12").
* **New: Aspect Ratio Control.** Crop gallery thumbnails to Square, 4:3, 16:9, 3:2, or 9:16 instead of relying on a fixed pixel height.
* **New: Right-Click Protection.** An optional setting prevents visitors from right-clicking to save gallery images.
* **Improved: Accessibility.** Gallery tiles, Polaroid cards, and both lightboxes are now keyboard-operable (Tab, Enter/Space, Escape, Arrow keys), carry proper `role`/`aria-label` attributes, and move focus into and back out of the lightbox correctly for screen reader and keyboard users.
